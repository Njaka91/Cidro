...
const PresidentCart = () => {
  const [refSmiro, inViewSmiro] = useInView({ triggerOnce: false, threshold: 0.2 });
  const [refSosm, inViewSosm] = useInView({ triggerOnce: false, threshold: 0.2 });
  const [refCfca, inViewCfca] = useInView({ triggerOnce: false, threshold: 0.2 });

  return (
    <div
      className="flex flex-row flex-wrap justify-around pb-12
      md:px-8 
      lg:px-24 
      xl:px-32"
    >
